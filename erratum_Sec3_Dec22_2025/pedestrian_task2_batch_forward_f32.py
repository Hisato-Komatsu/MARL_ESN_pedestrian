...
class Env:
   def __init__(self, hyperparameters, render_mode='off', experience_sharing='on'):
      ...
      self.agents[0].X_esn = np.zeros((self.agents[0].reservoir_size, self.n_agent_1group), dtype=np.float32)
      self.agents[self.n_agent_1group].X_esn = np.zeros((self.agents[self.n_agent_1group].reservoir_size, self.n_agent_1group), dtype=np.float32)
