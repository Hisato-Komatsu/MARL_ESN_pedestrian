...
class Env:
   def __init__(self, hyperparameters, render_mode='off'):
      ...
      self.agents[0].X_esn = np.zeros((self.n_agent, self.agents[0].reservoir_size), dtype=np.float32)
...
class Agent(Object):
   ...
   def choose_action(self, state_arr): #epsilon-greedy
      ...
      X_esn_candidates = X_esn_past + self.lr*(self.X_esn_tilde - X_esn_past) # T*a*Nres array
